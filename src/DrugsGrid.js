...
class DrugsGrid extends Component
{
    ...
    render(){
        ...
        {
            return (<Redirect to="/drugs/new"/>);
        }
        if(this.state.EditDrugClicked)
        {
            let url = `/drugs/edit/${this.SelectedId}`;
        ...
        {
            let url = `/drugs/details/${this.SelectedId}`;
            return (<Redirect to={url}/>);
        }
    ...
    {
        let thisObject = this;
        fetch(`${ConstantValues.WebApiBaseUrl}/api/crawler/drug/getall`,
        {
            method : "GET",
            headers:{
